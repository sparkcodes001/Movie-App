...
import { LogIn, Play, ShieldCheck, Activity, Terminal } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.1)_50%)] bg-[length:100%_4px] z-10 opacity-20" />
      ...
      {/* Main Content */}
      <main className="relative z-20 flex-1 container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 items-center gap-12 py-16 lg:py-0">
        <div className="hero-content flex flex-col items-center lg:items-start space-y-6 md:space-y-8">
          {/* Banner */}
          <div className="flex items-center gap-3 bg-white/[0.03] border border-white/10 px-4 py-2 backdrop-blur-md">
          ...
          <div className="space-y-3 text-center lg:text-left">
            <h1 className="text-6xl md:text-8xl lg:text-[7rem] font-black tracking-tighter leading-[0.8] uppercase">
            ...
            <div className="flex items-center gap-2 justify-center lg:justify-start">
              ...
              <p className="font-mono text-[9px] md:text-[10px] uppercase tracking-[0.3em] text-zinc-500">
          ...
          {/* Description */}
          <p className="max-w-[480px] text-zinc-400 text-sm md:text-base leading-relaxed font-medium text-center lg:text-left border-l-2 border-cyan-500/30 pl-5">
          ...
          {/* CTA */}
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:justify-start justify-center pt-2">
            <button
              onClick={handleAction}
              className={`group relative px-8 sm:px-10 py-4 sm:py-5 font-black uppercase italic tracking-widest text-xs transition-all active:scale-95 flex items-center gap-3 overflow-hidden ${
                hasAccess
                  ? "bg-cyan-500/10 border border-cyan-500/40 text-cyan-400 hover:text-white"
                  : "bg-white text-black"
              }`}
            >
              {/* Overlay */}
              <div className="absolute inset-0 bg-cyan-500 z-0 translate-y-full group-hover:translate-y-0 transition-transform duration-300" />
              <span className="relative z-10 flex items-center gap-3">
                {hasAccess && <ShieldCheck size={16} />}
                {hasAccess ? "Uplink Active" : "Initiate Breach"}
                <RiArrowRightUpLine
                  size={18}
        ...
        {/* Right panel - 3D Screen */}
        <div className="screen-container relative z-20 hidden lg:flex justify-center items-center">
          {/* HUD floating elements */}
          <div
            ref={(el) => (floatItemsRef.current[0] = el)}
            className="absolute -top-10 -left-10 z-30 font-mono text-[10px] text-cyan-500/50 flex flex-col gap-1 pointer-events-none"
          >
          ...
          <div
            ...
            className="absolute -bottom-10 -right-10 z-30 font-mono text-[10px] text-zinc-500 flex flex-col items-end pointer-events-none"
          >
          ...
          <div
            ref={screenRef}
            className="relative w-full max-w-[580px] aspect-video bg-zinc-900 rounded-lg border border-white/10 overflow-hidden group cursor-none shadow-[0_0_100px_rgba(0,0,0,1)] transform-gpu"
            style={{
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center z-10">
              <div className="relative p-6 rounded-full border border-white/10 bg-black/40 backdrop-blur-sm group-hover:bg-cyan-500 group-hover:border-cyan-400 group-hover:text-black transition-all duration-500">
                <Play fill="currentColor" size={36} className="ml-1" />
                <div className="absolute inset-0 rounded-full border border-cyan-500 animate-ping opacity-0 group-hover:opacity-50" />
              </div>
            </div>

            {/* Scanning line */}
            <div className="absolute top-0 left-0 w-full h-[2px] bg-cyan-500/30 shadow-[0_0_10px_cyan] animate-[scan_4s_linear_infinite] z-20" />
          </div>
        </div>
      </main>
